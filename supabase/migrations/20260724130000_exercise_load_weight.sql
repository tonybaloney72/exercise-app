-- Working load (lb) on exercise logs + Library default weight per exercise.

alter table public.exercise_logs
  add column if not exists actual_weight_lb numeric
    check (actual_weight_lb is null or actual_weight_lb > 0);

comment on column public.exercise_logs.actual_weight_lb is
  'Working load in pounds for this set (dumbbell/kettlebell/barbell/etc.).';

alter table public.exercise_settings
  add column if not exists default_weight_lb numeric
    check (default_weight_lb is null or default_weight_lb > 0);

comment on column public.exercise_settings.default_weight_lb is
  'Library default working load in pounds for loadable exercises.';
...
begin
  ...
  select
    ...
    nullif(el->>'actual_weight_lb', '')::numeric,
