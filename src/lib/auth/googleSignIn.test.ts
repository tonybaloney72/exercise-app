...
describe("buildOAuthCallbackUrl", () => {
  it("builds callback without next when returnTo is default home", () => {
    expect(buildOAuthCallbackUrl("https://myexercise.dev", "/today")).toBe(
  ...
  it("includes next for custom return paths", () => {
    expect(
      buildOAuthCallbackUrl("https://myexercise.dev", "/weekly"),
    ).toBe("https://myexercise.dev/auth/callback?next=%2Fweekly");
  });
});
