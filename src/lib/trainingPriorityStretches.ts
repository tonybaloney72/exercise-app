...
/** Whether a themed stretch pool applies for this day + priority weights. */
export function shouldIncludeStretchPool(
  pool: StretchThemePoolId,
  plan: DayPlan,
  categoriesInDay: ReadonlySet<ExerciseCategory>,
  preset: TrainingPriorityPreset,
  weights: TrainingPriorityWeights = weightsForPreset(preset),
): boolean {
  const hasUpper =
...
/** Warm-up stretch count for a themed pool (derived from priority weights). */
...
/** Cool-down stretch count for a themed pool (derived from priority weights). */
